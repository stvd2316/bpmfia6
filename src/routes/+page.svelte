			...
			<div class="calendar-grid">
				...
				{#if currentDate}
					...
					{#each Array.from({ length: dIM }) as _, i (i)}
						...
						<div class="cal-day {iT ? 'today' : ''} {iS ? 'selected' : ''} {iW ? 'weekend' : ''} {hA ? 'has-event' : ''}" onclick={() => handleDateClick(dO)} role="button">{i + 1}{#if hA}<span class="cal-dots" aria-hidden="true">{#each Array.from({ length: Math.min(cA, 15) }) as _, k (k)}<span class="cal-dot"></span>{/each}</span>{/if}</div>
					{/each}
				{/if}
			</div>
